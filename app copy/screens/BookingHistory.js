...
import { getOrders, getSP } from '../../api/api-orders';
...
export default function BookingUpcoming() {
  ...
  if (FetchingOrdersErr) {
    console.log(FetchingOrdersErr);
  }

  const { data: sp, error: FetchingSPsErr } = useQuery({
    queryFn: getSP,
    queryKey: ['sp'],
  });
  if (FetchingSPsErr) {
    console.log(FetchingSPsErr);
  }
              ...
              {bookings
                ...
                .map((booking) => (
                  ...
                  >
                    ...
                    <Text
                      style={{ color: 'white' }}
                    >{`${booking.description.slice(0, 40)}...`}</Text>
                    <Text
                      style={{
                        color: 'white',
                        padding: 6,
